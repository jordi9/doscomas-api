...
class NanoIdTest : StringSpec({
  ...
  "generate URL-friendly IDs with the requested prefix and default NanoID size" {
    val id = NanoId(Random(42)).get("sp_")
  ...
  "validate IDs by prefix, default NanoID size, and URL-friendly suffix" {
    ...
    NanoId.isValid("sp_", "sp_$validSuffix") shouldBe true
    NanoId.isValid("sp_", "acc_$validSuffix") shouldBe false
    NanoId.isValid("sp_", "sp_${"a".repeat(20)}") shouldBe false
    NanoId.isValid("sp_", "sp_${"a".repeat(22)}") shouldBe false
    NanoId.isValid("sp_", "sp_${"a".repeat(20)}!") shouldBe false
  }
  ...
  "validate every URL-friendly alphabet character" {
    ...
    alphabet.forEach { char ->
      NanoId.isValid("sp_", "sp_${char.toString().repeat(21)}") shouldBe true
    }

    "+/=:!".forEach { char ->
      NanoId.isValid("sp_", "sp_${char.toString().repeat(21)}") shouldBe false
    }
  }
})
